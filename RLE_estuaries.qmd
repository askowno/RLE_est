...
est_lc_metrics <- est_t2 %>%
  ...
  mutate(nat2040 = nat1990 - (ard9022*50))  %>%
  mutate(prplost9040 = (nat1990-nat2040)/nat1990) %>%
  # assessment
  ...
  mutate(A2b_24 = case_when(prplost9040 >= 0.8 ~ "CR",
                         prplost9040 >= 0.5 ~ "EN",
                         prplost9040 >= 0.3 ~ "VU", 
                         TRUE ~ "LC")) %>%
...
#  B1i using ongoing decline  
  ...
  # add the table with A3 and Asb
    ...
    relocate(D3_18, B1aii_18, D3_24, B1aii_24, A2b_24, A3_24, .after = prplost9040) %>%
  # overall RLE outcome
     rowwise() %>%
   mutate(RLE24 = max(c_across(all_of(c("D3_24", "B1aii_24", "A2b_24", "A3_24"))), na.rm = TRUE)) %>%
     ungroup() %>%
    rowwise() %>%
   mutate(RLE18 = max(c_across(all_of(c("D3_18", "B1aii_18", "A2b_24", "A3_24"))), na.rm = TRUE)) %>%
     ungroup()
